...
for d in data_sets:
    for m in methods:
        data_df = pd.read_csv('heatmap/COVID_single_z_'+m+'_'+d+'.txt', delimiter = "\t",index_col=0)
        data_df = data_df.loc[:,(data_df != 0).any(axis=0)]
        data_df.replace(0, np.nan, inplace=True)
        count_df = pd.read_csv('heatmap/COVID_single_count_'+m+'_'+d+'.txt', delimiter = "\t",index_col=0)
        exist_df = pd.read_csv('heatmap/COVID_single_exist_'+m+'_'+d+'.txt', delimiter = "\t",index_col=0)
        ...
        ax2.set_title('')
        plt.savefig('heatmap/COVID_single_z_'+m+'_'+d+'.png',bbox_inches='tight')
        plt.show()
        ...
        plt.ylabel('')
        plt.xlabel('')
        plt.savefig('heatmap/COVID_single_count_'+m+'_'+d+'.png',bbox_inches='tight')
        plt.show()
        ...
        plt.savefig('heatmap/COVID_single_exist_'+m+'_'+d+'.png',bbox_inches='tight')
        plt.show()
#%%
...
#cb.ax.set_xticklabels([0,1,2,3,4,5,6])
cb.set_label(label='Association\nSignificance (z-score)',size=25, weight='bold')
plt.savefig('D:/experiment/COVID/heatmap/just_colorbar.png', bbox_inches='tight')
plt.show()
